...
To develop this project you'll need to at least have [`git`][git] and [`rustup`][rustup] installed. [`cargo-expand`][cargo-expand] is not required, but we recommended installing it because [tests that check the code generated by macros](tests/expand/README.md) require it.

`dev.sh` runs the simplified version of the checks done by CI. We recommend making this check before every pull request (and every new commit in a pull request).

```sh
./dev.sh
```

This check is almost equivalent to running `cargo test --all && cargo fmt --all && cargo clippy --all --all-targets -Z unstable-options && cargo doc --no-deps --all` with nightly toolchain, but it automates set up the required toolchain and components ([rustfmt], [clippy], etc.).
